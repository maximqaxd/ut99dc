...
{ \
	...
	if( !Owner ) \
		if \
		(   !ClipTo( FPlane( 0.0, 0.0,-1.0,0.1-Box.Min.Z), INDEX_NONE ) \
		||	!ClipTo( FPlane( 0.0, 0.0,+1.0,Box.Max.Z+0.1), INDEX_NONE ) \
		||	!ClipTo( FPlane(-1.0, 0.0, 0.0,0.1-Box.Min.X), INDEX_NONE ) \
		||	!ClipTo( FPlane(+1.0, 0.0, 0.0,Box.Max.X-0.1), INDEX_NONE ) \
		||  !ClipTo( FPlane( 0.0,-1.0, 0.0,0.1-Box.Min.Y), INDEX_NONE ) \
		||	!ClipTo( FPlane( 0.0,+1.0, 0.0,Box.Max.Y-0.1), INDEX_NONE ) ) \
			goto NoBlock; \
	\
